...
public class StorageUtilsIT extends IntegrationTest
{
	...
	public void testgetOptional()
	{
		...
		TestLibraryPatron from_storage = StorageUtils.getOptional(TestLibraryPatron.KIND, patron_in_storage.getSimpleObjectId(), null);
		assertTrue(patron_in_storage.equals(from_storage));

		// Remove the entry ensure that are method gets the update and we no longer can
		// grab the object, it's only in RAM now
		CloudExecutionEnvironment.getSimpleCurrent().getSimpleStorage().delete(patron_in_storage);
		from_storage = StorageUtils.getOptional(TestLibraryPatron.KIND, patron_in_storage.getSimpleObjectId(), null);
		assertTrue(!patron_in_storage.equals(from_storage));

		// Ensure that we can create an object, not put in Storage, and then be unable
		// to get it
		...
		from_storage = StorageUtils.getOptional(TestLibraryPatron.KIND, patron_not_in_storage.getSimpleObjectId(), null);
