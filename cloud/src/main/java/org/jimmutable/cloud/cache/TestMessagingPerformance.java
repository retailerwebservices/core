...
public class TestMessagingPerformance
{
	...
	static public void main( String args[] )
	{
		...
		try
		{
			...
			if ( test.equalsIgnoreCase("signal") && mode.equalsIgnoreCase("source") )
				testSignalSource(host, port);
			if ( test.equalsIgnoreCase("signal") && mode.equalsIgnoreCase("sink") )
				testSignalSink(host, port);
			if ( test.equalsIgnoreCase("queue") && mode.equalsIgnoreCase("source") )
				testQueueSource(host, port);
			if ( test.equalsIgnoreCase("queue") && mode.equalsIgnoreCase("sink") )
				testQueueSink(host, port);
		}
		catch ( Exception e )
		{
	...
	static public class SinkSignalStatusPrinter implements Sink<Integer>
	{
		...
		public void onEmit( Integer count )
		{
			int processed_in_last_interval = listener.count
					- last_count;
			long time_in_last_interval = System.currentTimeMillis()
					- last_time;
	...
	static public Options createOptions()
	{
		...
		Option test = Option.builder("test").hasArg().desc("specify which type of test to run").argName("signal | queue").build();

		Option mode = Option.builder("mode").hasArg().desc("specify if the instance is to generate messages or process them").argName("source | sink").build();
	...
	/*
	 * static public void run() { CloudExecutionEnvironment.startupStubTest(new
	 * ApplicationId("test"));
	 * 
	 * String host = NetUtils.extractHostFromHostPortPair(System.getenv(
	 * "jimmutable_messaging_server"), "localhost"); int port =
	 * NetUtils.extractPortFromHostPortPair(System.getenv(
	 * "jimmutable_messaging_server"), LowLevelRedisDriver.DEFAULT_PORT_REDIS);
	 * 
	 * System.out.println("Starting test of messaging on "+host+":"+
	 * port+" in 2 seconds");
	 * 
	 ...
	 * (host, port));
	 * 
	 * emitter = RateLimitingEmitter.startEmitter(new MessageSource(), new
	 * MessageSink(), 1.0f);
	 * 
	 * MessageListener listener;
	 * 
	 * for ( int i = 0; i < 10; i++ ) { listener = new MessageListener();
