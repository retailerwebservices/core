...
/**
 * Interface defines a shared application cache (e.g. a redis cache)
 * 
 * Caches are LRU and will (memory permitting) drop items. As a result, it is
 * never safe to assume that anything added to a cache will be available at a
 * later time (caches are volatile)
 * 
 * @author kanej
 *
 */
public interface Cache
{
	 ...
	 * 
	 * @param prefix The prefix of the entries to delte
	 */
	default public void deleteAllSlow(CacheKey prefix) { scan(prefix, ScanOperationDeleteAll.OPERATION); }
	
	/**
	 * Perform an operation on all keys in the cache with a given prefix
	 * 
	 * @param prefix
	 *            The prefix of the key(s) to perform the operation on
	 * @param operation
	...
	default public long getTTL(String key, long default_value) { return getTTL(new CacheKey(key), default_value); }
